...
class M:
    ...
    def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False):
        ...
        # input image batch
        self.inputImgs = tf.compat.v1.placeholder(tf.float32, shape=(None, M.imgSize[0], M.imgSize[1]))
    ...
    def setupCNN(self):
        ...
        with tf.name_scope('Conv_Pool_1'):
            kernel = tf.Variable(
                tf.compat.v1.truncated_normal([5, 5, 1, 64], stddev=0.1))
            conv = tf.nn.conv2d(
    ...
    def setupTF(self):
        ...
        modelDir = '../model/'
        latestSnapshot = tf.compat.v1.train.latest_checkpoint(modelDir)  # Is there a saved model?
    ...
    def decoderOutputToText(self, ctcOutput):
        ...
        encodedLabelStrs = [[] for i in range(M.batchSize)]
        ...
        # TF decoders: label strings are contained in sparse tensor
            ...
            idxDict = {b : [] for b in range(M.batchSize)}
    ...
    def trainBatch(self, batch, batchNum):
        ...
        feedDict = {self.inputImgs: batch.imgs, self.gtTexts: sparse, self.seqLen: [M.maxTextLen] * M.batchSize, self.learningRate: rate}
    ...
    def return_rnn_out(self, batch, write_on_csv=False):
        ...
        decoded, rnnOutput = self.sess.run([self.decoder, self.ctcIn3dTBC],
                                {self.inputImgs: batch.imgs, self.seqLen: [M.maxTextLen] * numBatchElements})
    ...
    def inferBatch(self, batch):
        ...
        numBatchElements = len(batch.imgs)
        feedDict = {self.inputImgs: batch.imgs, self.seqLen: [M.maxTextLen] * numBatchElements}
