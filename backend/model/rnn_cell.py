...
class LSTMAttentionCell(tf.compat.v1.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.compat.v1.variable_scope(scope or type(self).__name__, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            phi_flat = tf.reduce_sum(alpha*tf.exp(-1 * tf.square(kappa - u) / beta), axis=1)
    ...
    def output_function(self, state):
        ...
        mvn = tfd.distributions.MultivariateNormalFullCovariance(loc=mus, covariance_matrix=covar_matrix)
        b = tfd.distributions.Bernoulli(probs=es)
        c = tfd.distributions.Categorical(probs=pis)
        ...
        return tf.concat([coords, tf.cast(sampled_e, tf.float32)], 1)
